...
function createMitmProxyManager(deps = {}) {
  ...
  // ---- 常驻待机代理：保持端口永远在线，避免手机端代理判定失败后回退直连并缓存 ----
  // 空闲时待机代理对所有连接做直通转发（不解密、不抓取）；
  // 抓包会话启动时接管端口做 MITM，会话结束后自动恢复待机监听。
  let standbyEntry = null; // { servers }
  let standbyRestartTimer = null;
  ...
  async function stopStandby() {
    if (standbyRestartTimer) {
      clearTimeout(standbyRestartTimer);
      standbyRestartTimer = null;
    }
    const entry = standbyEntry;
    standbyEntry = null;
    if (!entry) return;
    await Promise.all(entry.servers.map(closeServer));
  }

  async function startStandby() {
    if (standbyEntry) return;
    ...
    try {
      for (const bindIp of bindTargets) {
        const server = net.createServer(socket => handleStandbyClient(socket));
        server.on('error', () => {});
      ...
      log('warn', `待机代理启动失败（端口可能被会话代理占用）: ${error.message}`);
      return;
    }
    standbyEntry = { servers };
    log('info', `待机代理已常驻: 端口 ${port}（空闲时直通转发，会话启动时切换抓包）`);
  }

  /** 会话代理停止后延迟恢复待机监听，避免端口释放竞态 */
  function scheduleStandbyRestart(delayMs = 1500) {
    if (standbyRestartTimer) clearTimeout(standbyRestartTimer);
    standbyRestartTimer = setTimeout(() => {
      standbyRestartTimer = null;
      void startStandby();
    }, delayMs);
    if (standbyRestartTimer.unref) standbyRestartTimer.unref();
  }

  /** 待机模式客户端处理：CONNECT 直通隧道，明文绝对 URL 直接转发 */
  function handleStandbyClient(rawSocket) {
    ...
    const onData = (chunk) => {
      ...
      if (connect) {
        startTunnel(rawSocket, connect, rest);
        return;
      }
      ...
      if (absolute) {
        standbyForwardPlain(rawSocket, absolute, head, rest);
        return;
      }
  ...
  function standbyForwardPlain(rawSocket, { host, port, path }, head, rest) {
